...
def feature_visualization(x, module_type, stage, n=32, save_dir="runs/detect/exp"):
    """
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            ...
            # print("blocks[0].squeeze() = ", blocks[0].squeeze())
            n = min(n, channels)  # number of plots
            fig, ax = plt.subplots(math.ceil(n / 8), 8, tight_layout=True)  # 8 rows x n/8 cols， math.ceil向上取整
            ax = ax.ravel()
            ...
            # np.save(str(Path(f).with_suffix('.npy')), x[0].cpu().numpy())  # npy save
...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3):
    ...
    if label:
        ...
        return code_1, code_2, exi, label  # 左上角坐标，右下角坐标，存在目标标志， 目标类型
